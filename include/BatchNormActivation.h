				...
				for_i(C, [=](UInt c)
				{
					...
					if (InplaceBwd)
						for (auto n = 0ull; n < batchSize; n++)
						{
							...
							for (auto hw = start; hw < part; hw += VectorSize)
							{
								diffSrc = Activation::dfVec((VecFloat().load_a(&InputsOriginal[0]->Neurons[hw]) - Mean[c]) * weightedInvStdDev + biases) * VecFloat().load_a(&InputLayer->NeuronsD1[hw]);
						...
						for (auto n = 0ull; n < batchSize; n++)
						{
							...
							for (auto hw = start; hw < part; hw += VectorSize)
							{
								diffSrc = Activation::dfVec((VecFloat().load_a(&InputsOriginal[0]->Neurons[hw]) - Mean[c]) * weightedInvStdDev + biases) * VecFloat().load_a(&NeuronsD1[hw]);
								diffGamma = mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[hw]) - Mean[c], diffSrc, diffGamma);
								diffBeta += diffSrc;
							}
							for (auto hw = part; hw < start + HW; hw++)
							{
								diffSrcFloat = Activation::df(((InputsOriginal[0]->Neurons[hw] - Mean[c]) * weightedInvStdDev) + biases) * NeuronsD1[hw];
					...
					if (InplaceBwd)
						for (auto n = 0ull; n < batchSize; n++)
						{
							...
							for (auto hw = part; hw < start + HW; hw++)
							{
								...
								diffSrcFloat = Activation::df(((InputsOriginal[0]->Neurons[hw] - Mean[c]) * weightedInvStdDev) + biases) * InputLayer->NeuronsD1[hw];
					...
					if (InplaceBwd)
						for (auto n = 0ull; n < batchSize; ++n)
						{
							...
							for (auto h = 0ull; h < H; ++h)
							{
								...
								for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								{
									diffSrc = Activation::dfVec(mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[w]) - mean, weightedInvStdDev, biases)) * VecFloat().load_a(&InputLayer->NeuronsD1[w]);
						...
						for (auto n = 0ull; n < batchSize; ++n)
						{
							...
							for (auto h = 0ull; h < H; ++h)
							{
								...
								for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								{
									diffSrc = Activation::dfVec(mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[w]) - mean, weightedInvStdDev, biases)) * VecFloat().load_a(&NeuronsD1[w]);
					...
					if (InplaceBwd)
						for (auto n = 0ull; n < batchSize; ++n)
						{
							...
							for (auto h = 0ull; h < H; ++h)
							{
								...
								for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								{
									diffSrc = mul_add(Activation::dfVec(mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[w]) - mean, weightedInvStdDev, biases)), VecFloat().load_a(&InputLayer->NeuronsD1[w]), -mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[w]) - mean, diffGamma, diffBeta));
						...
						for (auto n = 0ull; n < batchSize; ++n)
						{
							...
							for (auto h = 0ull; h < H; ++h)
							{
								...
								for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								{
									diffSrc = mul_add(Activation::dfVec(mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[w]) - mean, weightedInvStdDev, biases)), VecFloat().load_a(&NeuronsD1[w]), -mul_add(VecFloat().load_a(&InputsOriginal[0]->Neurons[w]) - mean, diffGamma, diffBeta));
