...
namespace dnn
{
	...
	public:
		...
		BatchNormActivation<Activation,T>(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector< std::shared_ptr<Layer>>& inputs, const bool scaling = true, const Float momentum = Float(0.99), const Float eps = Float(1e-04), const bool hasBias = true) :
			...
			if (IsPlainFormat())
			{
				...
				for_i(C, [=](size_t c)
				{
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto i = start; i < part; i += VectorSize)
						{
							diffSrc = Activation::dfVec(VecFloat().load_a(&Neurons[i])) * VecFloat().load_a(&NeuronsD1[i]);
						...
						for (auto i = part; i < end; i++)
						{
							diffSrcFloat = Activation::df(Neurons[i]) * NeuronsD1[i];
