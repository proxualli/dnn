...
namespace dnn
{
	...
	{
	private:
		std::unique_ptr<dnnl::softmax_forward::primitive_desc> fwdDescLogSoftmax;
		std::unique_ptr<dnnl::softmax_backward::primitive_desc> bwdDescLogSoftmax;
	...
	public:
		...
		void InitializeDescriptors(const UInt batchSize) final override
		{
			if (InputLayer->DstMemDesc->get_ndims() == 2)
			{
				...
				InputLayerDiffDstMemDesc = std::make_unique<dnnl::memory::desc>(dnnl::memory::desc(dnnl::memory::dims({ dnnl::memory::dim(batchSize), dnnl::memory::dim(InputLayer->C) }), dnnl::memory::data_type::f32, ChosenFormat));
			}
			else
			{
			...
			fwdDescLogSoftmax = std::make_unique<dnnl::softmax_forward::primitive_desc>(dnnl::softmax_forward::primitive_desc(Device.engine, dnnl::prop_kind::forward, dnnl::algorithm::softmax_log, *InputLayerDstMemDesc, *DstMemDesc, axis));
			bwdDescLogSoftmax = std::make_unique<dnnl::softmax_backward::primitive_desc>(dnnl::softmax_backward::primitive_desc(Device.engine, dnnl::algorithm::softmax_log, *InputLayerDiffDstMemDesc, *DiffDstMemDesc, *DstMemDesc, axis, *fwdDescLogSoftmax));
...
#ifdef DNN_CACHE_PRIMITIVES
			fwdLogSoftmax = std::make_unique<dnnl::softmax_forward>(dnnl::softmax_forward(*fwdDescLogSoftmax));
			bwdLogSoftmax = std::make_unique<dnnl::softmax_backward>(dnnl::softmax_backward(*bwdDescLogSoftmax));
#endif
		}
...
#else
			dnnl::softmax_forward(*fwdDescLogSoftmax).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem}, { DNNL_ARG_DST, dstMem } });
#endif
			Device.stream.wait();
...
#else
			dnnl::softmax_backward(*bwdDescLogSoftmax).execute(Device.stream, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_DST, dstMem}, { DNNL_ARG_DIFF_DST, diffDstMem }, { DNNL_ARG_DIFF_SRC, diffSrcMem } });
#endif
			Device.stream.wait();
