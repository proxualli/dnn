...
namespace dnn
{
	class ReductionMin final : public Layer
	{
	private:
	...
	public:
		ReductionMin(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs) :
			Layer(device, format, name, LayerTypes::ReductionMin, 0, 0, 1, inputs[0]->D, inputs[0]->H, inputs[0]->W, 0, 0, 0, inputs)
		{
		}
		...
		void InitializeDescriptors(const UInt batchSize)  final override
		{
			...
			fwdDescReduction = std::make_unique<dnnl::reduction::primitive_desc>(dnnl::reduction::primitive_desc(Device.engine, dnnl::algorithm::reduction_min, *InputLayer->DstMemDesc, *DstMemDesc, 0.f, 0.f));
#ifdef DNN_CACHE_PRIMITIVES
					...
					for_i(batchSize, threads, [=](UInt n)
					{
						...
						for (auto c = 0ull; c < PaddedC; c += VectorSize)
						{
							...
							for (auto hw = 0ull; hw < strideHW; hw += VectorSize)
							{
								...
								Out.load_a(&Neurons[hw]);
								OutD1.load_a(&NeuronsD1[hw]);
								InD1 += select(In == Out, OutD1, Float(0));
					...
					for_i(batchSize, threads, [=](UInt n)
					{
						...
						for (auto c = 0ull; c < PaddedC; c += VectorSize)
						{
							...
							for (auto hw = 0ull; hw < strideHW; hw += VectorSize)
							{
								In.load_a(&InputLayer->Neurons[hw + inputOffset]);
								InD1.load_a(&InputLayer->NeuronsD1[hw + inputOffset]);
								Out.load_a(&Neurons[hw + outputOffset]);
