...
namespace dnn
{
	...
	public:
		...
		Cost(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const Costs cost, const size_t groupIndex, const size_t labelIndex, const size_t c, const std::vector<std::shared_ptr<Layer>>& inputs, const Float labelTrue, const Float labelFalse, const Float weight, const Float eps) :
