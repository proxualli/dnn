...
namespace dnn
{
	class ReductionMax final : public Layer
	{
	private:
	...
	public:
		ReductionMax(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs) :
			Layer(device, format, name, LayerTypes::ReductionMax, 0, 0, 1, inputs[0]->D, inputs[0]->H, inputs[0]->W, 0, 0, 0, inputs)
		{
		}
		...
		void InitializeDescriptors(const UInt batchSize)  final override
		{
			...
			fwdDescReduction = std::make_unique<dnnl::reduction::primitive_desc>(dnnl::reduction::primitive_desc(Device.engine, dnnl::algorithm::reduction_max, *InputLayer->DstMemDesc, *DstMemDesc, 0.f, 0.f));
#ifdef DNN_CACHE_PRIMITIVES
