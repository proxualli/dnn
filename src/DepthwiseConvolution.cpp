...
namespace dnn
{
	...
	std::string DepthwiseConvolution::GetDescription() const
	{
	...
	size_t DepthwiseConvolution::FanIn() const
	{
		return KernelH * KernelW;
	}

	size_t DepthwiseConvolution::FanOut() const
	{
		return Multiplier * KernelH * KernelW / StrideH * StrideW;
	}

	void DepthwiseConvolution::InitializeDescriptors(const size_t batchSize)
	{
	...
	void DepthwiseConvolution::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void DepthwiseConvolution::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
	...
	ByteVector DepthwiseConvolution::GetImage(const Byte fillColor)
	{
