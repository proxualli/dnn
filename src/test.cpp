...
DNN_API void DNNSetNewEpochDelegate(void(*newEpoch)(UInt, UInt, UInt, UInt, Float, Float, bool, bool, Float, Float, bool, Float, Float, UInt, Float, UInt, Float, Float, Float, UInt, UInt, UInt, Float, Float, Float, Float, Float, Float, UInt, Float, Float, Float, UInt));
...
#endif
{
    ...
    if (DNNReadDefinition(model, msg) == 1)
    {
        if (DNNLoadDataset())
        {
            ...
            DNNAddLearningRateSGDR(true, 1, *trainingSamples, Optimizers::NAG, 0.9f, 0.999f, 0.0005f, 0.0f, 0.00001f, 128, 128, 128, 1, 200, 1, 0.05f, 0.0001f, 0.1f, 0.003f, 1, 1.0f, true, false, 0.0f, 0.7f, true, 0.7f, 0.7f, 20, 0.7f, Interpolations::Cubic, 10.0f, 12.0f);
            DNNTraining();
