...
namespace dnn
{
	Multiply::Multiply(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs) :
	...
	std::string Multiply::GetDescription() const
	{
		return GetDescriptionHeader();
	}

	size_t Multiply::FanIn() const
	{
		return 1;
	}

	size_t Multiply::FanOut() const
	{
		return 1;
	}

	void Multiply::InitializeDescriptors(const size_t batchSize)
	{
	...
	void Multiply::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void Multiply::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
