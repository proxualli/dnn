...
namespace dnn
{
	Model::Model(const char* name, Dataprovider* dataprovider) :
		Name(name),
	...
	bool Model::IsUniqueLayerName(const char* name) const
	{
	...
	void Model::SetHyperParameters(const Float adaDeltaEps, const Float adaGradEps, const Float adamEps, const Float adamBeta2, const Float adamaxEps, const Float adamaxBeta2, const Float rmsPropEps, const Float radamEps, const Float radamBeta1, const Float radamBeta2)
	{
		for (auto layer : Layers)
		{
	...
	std::vector<Layer*> Model::GetLayerInputs(const std::vector<std::string>& inputs) const
	{
	...
	std::vector<Layer*> Model::GetLayerOutputs(const Layer* layer) const
	{
		auto list = std::vector<Layer*>();
	...
	std::vector<Layer*> Model::SetRelations()
	{
	...
	void Model::SetLocking(const bool locked)
	{
	...
	void Model::SetLayerLocking(const size_t layerIndex, const bool locked)
	{
	...
	size_t Model::GetWeightsSize(const bool persistOptimizer) const
	{
	...
	size_t Model::GetNeuronsSize(const size_t batchSize) const
	{
	...
	bool Model::BatchNormalizationUsed() const
	{
		for (auto layer : Layers)
	...
	void Model::AddTrainingRate(const TrainingRate rate, const bool clear, const size_t gotoEpoch)
	{
		if (clear)
			TrainingRates.clear();
	...
	void Model::AddTrainingRateSGDR(const TrainingRate rate, const bool clear, const size_t gotoEpoch)
	{
		if (clear)
			TrainingRates.clear();
	...
	bool Model::CheckDefinition(std::string& definition, CheckMsg& checkMsg)
	{
		definition = Definition::Normalize(definition.c_str());

		Definition::Read(definition.c_str(), checkMsg, true);

		return checkMsg.Error;
	}

	Model* Model::ReadDefinition(const char* definition, const Optimizers optimizer, Dataprovider* dataprovider, CheckMsg& checkMsg)
	{
		Model* model = Definition::Read(Definition::Normalize(definition).c_str(), checkMsg, false, dataprovider, optimizer);
	...
	Model* Model::LoadDefinition(const char* fileName, const Optimizers optimizer, Dataprovider* dataprovider, CheckMsg& checkMsg)
	{
		Model* model = nullptr;
		...
		if (!file.bad() && file.is_open())
		{
			...
			file.close();
			model = Model::ReadDefinition(buffer.str().c_str(), optimizer, dataprovider, checkMsg);
		}

		return model;
	}

	void Model::ForwardProp(const size_t batchSize)
	{
		for (size_t i = 0; i < Layers.size(); i++)
			Layers[i]->ForwardProp(batchSize, State.load() == States::Training);
	}

	void Model::BackwardProp(const size_t batchSize)
	{
	...
	bool Model::CheckTaskState() const
	{
	...
	void Model::TrainingAsync()
	{
		task = std::async(std::launch::async, [=] { Training(); });
	}

	void Model::TestingAsync()
	{
	...
	void Model::PauseTask()
	{
		if (TaskState.load() == TaskStates::Running)
			TaskState.store(TaskStates::Paused);
	}

	void Model::ResumeTask()
	{
		if (TaskState.load() == TaskStates::Paused)
			TaskState.store(TaskStates::Running);
	}
	
	void Model::SetOptimizer(const Optimizers optimizer)
	{
		for (auto layer : Layers)
	...
	void Model::CostFunction(const States state)
	{
	...
	void Model::Recognized(const States state, const std::vector<size_t>& sampleLabel)
	{
		for (auto cost : CostLayers)
		{
	...
	void Model::CostFunctionBatch(const States state, const size_t batchSize, const bool overflow, const size_t skipCount)
	{
		for (auto cost : CostLayers)
		{
	...
	void Model::RecognizedBatch(const States state, const size_t batchSize, const bool overflow, const size_t skipCount, const std::vector<std::vector<size_t>>& sampleLabels)
	{
		for (auto cost : CostLayers)
		{
			const auto inputLayer = cost->InputLayer;
	...
	void Model::SetBatchSize(const size_t batchSize)
	{
		if (!BatchSizeChanging.load() && !ResettingWeights.load())
		{
	...
	bool Model::GetInputSnapShot(std::vector<Float>* snapshot, std::vector<size_t>* label)
	{
...
#ifdef DNN_STOCHASTIC
	std::vector<size_t> Model::TrainSample(const size_t index)
	{
	...
	std::vector<size_t> Model::TestSample(const size_t index)
	{
	...
	std::vector<size_t> Model::TestAugmentedSample(const size_t index)
	{
		auto SampleLabel = DataProv->TestingLabels[index];
	...
	std::vector<std::vector<size_t>> Model::TrainBatch(const size_t index, const size_t batchSize)
	{
	...
	std::vector<std::vector<size_t>> Model::TestBatch(const size_t index, const size_t batchSize)
	{
		auto SampleLabels = std::vector<std::vector<size_t>>(batchSize, std::vector<size_t>(DataProv->Hierarchies));
	...
	std::vector<std::vector<size_t>> Model::TestAugmentedBatch(const size_t index, const size_t batchSize)
	{
		auto SampleLabels = std::vector<std::vector<size_t>>(batchSize, std::vector<size_t>(DataProv->Hierarchies));
