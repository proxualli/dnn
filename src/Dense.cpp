...
namespace dnn
{
	Dense::Dense(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const size_t c, const std::vector<Layer*>& inputs, const bool hasBias) :
	...
	std::string Dense::GetDescription() const
	{
	...
	size_t Dense::FanIn() const
	{
		return InputLayer->CDHW;
	}

	size_t Dense::FanOut() const
	{
		return CDHW;
	}

	void Dense::InitializeDescriptors(const size_t batchSize)
	{
	...
	void Dense::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void Dense::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
	...
	ByteVector Dense::GetImage(const Byte fillColor)
	{
