...
namespace dnn
{
	...
	std::string Convolution::GetDescription() const
	{
		...
		description.append(nwl + " Connections:" + tab + std::to_string(Groups * (InputLayer->C / Groups) * (C / Groups) * (H / StrideW) * (W / StrideH) * (HasBias ? KernelH * KernelW + 1 : KernelH * KernelW)));
	
		return description;
	}

	size_t Convolution::FanIn() const
	{
		return InputLayer->C / Groups * KernelH * KernelW;
	}

	size_t Convolution::FanOut() const
	{
		return C / Groups * KernelH * KernelW / StrideH * StrideW;
	}

	void Convolution::InitializeDescriptors(const size_t batchSize)
	{
	...
	void Convolution::ForwardProp(const size_t batchSize, const bool training)
	{
...
#ifdef DNN_CACHE_PRIMITIVES
		HasBias ? fwd->execute(Device.second, std::unordered_map<int, dnnl::memory>{ {DNNL_ARG_SRC, srcMem} , {DNNL_ARG_WEIGHTS, weightsMem}, {DNNL_ARG_BIAS, dnnl::memory(fwdDesc->bias_desc(), Device.first, Biases.data())}, {DNNL_ARG_DST, dstMem} }) :
	...
	void Convolution::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
	...
	ByteVector Convolution::GetImage(const Byte fillColor)
	{
		...
		if (Groups > 1)
		{
			...
			for (auto g = 0ull; g < Groups; g++)
			{
				for (auto c = 0ull; c < (C / Groups); c++)
				{
					...
					if (HasBias)
						image[left + biasOffset] = GetColorFromRange(rangeBiases, BiasesMin, Biases[g * (C / Groups) + c]);
				}
			}
