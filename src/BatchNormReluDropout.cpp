...
namespace dnn
{
	BatchNormReluDropout::BatchNormReluDropout(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const Float dropout, const bool scaling, const Float momentum, const Float eps, const bool hasBias) :
		Layer(device, format, name, LayerTypes::BatchNormReluDropout, inputs[0]->C, inputs[0]->C, inputs[0]->C, inputs[0]->D, inputs[0]->H, inputs[0]->W, 0, 0, 0, inputs, hasBias),
		Scaling(scaling),
		Eps(eps),
		Momentum(momentum),
	...
	std::string BatchNormReluDropout::GetDescription() const
	{
	...
	size_t BatchNormReluDropout::FanIn() const
	{
		return 1;
	}

	size_t BatchNormReluDropout::FanOut() const
	{
		return 1;
	}

	void BatchNormReluDropout::InitializeDescriptors(const size_t batchSize)
	{
	...
	void BatchNormReluDropout::SetBatchSize(const size_t batchSize)
	{
	...
	void BatchNormReluDropout::ForwardProp(const size_t batchSize, const bool training)
	{
		...
		if (!training)
		{
			if (Scaling)
			{
				...
				{
					const auto runningMean = VecFloat().load_a(&RunningMean[c * VectorSize]);
					const auto invStdDev = VecFloat(1) / sqrt(VecFloat().load_a(&RunningVariance[c * VectorSize]) + Eps);

					const auto weightedInvStdDev = invStdDev * VecFloat().load_a(&Weights[c * VectorSize]);
					const auto biases = HasBias ? VecFloat().load_a(&Biases[c * VectorSize]) : VecFloat(0);

					const auto mapOffset = c * VectorSize * HW;
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								Relu::fVec(mul_add(VecFloat().load_a(&InputLayer->Neurons[w]) - runningMean, weightedInvStdDev, biases)).store_a(&Neurons[w]);
						}
					}
				});
			}
				...
				for_i(PaddedC / VectorSize, [=](size_t c)
				{
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							const auto offsetH = offsetC + h * strideH;
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
								Relu::fVec((VecFloat().load_a(&InputLayer->Neurons[w]) - runningMean) * invStdDev).store_a(&Neurons[w]);
						}
					}
				});
			}
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
							{
								...
								(neuronsActive * Scale * Relu::fVec(mul_add(VecFloat().load_a(&InputLayer->Neurons[w]) - mean, weightedInvStdDev, biases))).store_a(&Neurons[w]);
#ifndef DNN_LEAN
					...
					for (auto n = 0ull; n < batchSize; n++)
					{
						...
						for (auto h = 0ull; h < H; h++)
						{
							...
							for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
							{
								...
								(neuronsActive * Scale * Relu::fVec((VecFloat().load_a(&InputLayer->Neurons[w]) - mean) * invStdDev)).store_a(&Neurons[w]);
#ifndef DNN_LEAN
								vecZero.store_nt(&NeuronsD1[w]);
#endif
							}
	...
	void BatchNormReluDropout::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
		...
		for_i(PaddedC / VectorSize, [=](size_t c)
		{
			...
			for (auto n = 0ull; n < batchSize; ++n)
			{
				...
				for (auto h = 0ull; h < H; ++h)
				{
					...
					for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
					{
						diffSrc = VecFloat().load_a(&NeuronsActive[w]) * Relu::dfVec(VecFloat().load_a(&Neurons[w])) * VecFloat().load_a(&NeuronsD1[w]);
			...
			for (auto n = 0ull; n < batchSize; ++n)
			{
				...
				for (auto h = 0ull; h < H; ++h)
				{
					...
					for (auto w = offsetH; w < offsetH + strideH; w += VectorSize)
					{
						...
						//(VecFloat().load_a(&InputLayer->NeuronsD1[w]) += diffSrc).store_a(&InputLayer->NeuronsD1[w]);
					}
				}
			}
		});
	...
	ByteVector BatchNormReluDropout::GetImage(const Byte fillColor)
	{
		if (Scaling)
		{
