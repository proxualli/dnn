...
namespace dnn
{
	LocalResponseNormalization::LocalResponseNormalization(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const bool acrossChannels, const size_t localSize, const Float alpha, const Float beta, const Float k) :
	...
	std::string LocalResponseNormalization::GetDescription() const
	{
	...
	size_t LocalResponseNormalization::FanIn() const
	{
		return 1;
	}

	size_t LocalResponseNormalization::FanOut() const
	{
		return 1;
	}

	void LocalResponseNormalization::InitializeDescriptors(const size_t batchSize)
	{
	...
	void LocalResponseNormalization::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void LocalResponseNormalization::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
