...
namespace dnn
{
	ChannelShuffle::ChannelShuffle(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const size_t groups) :
	...
	std::string ChannelShuffle::GetDescription() const
	{
	...
	size_t ChannelShuffle::FanIn() const
	{
		return 1;
	}

	size_t ChannelShuffle::FanOut() const
	{
		return 1;
	}

	void ChannelShuffle::InitializeDescriptors(const size_t batchSize)
	{
	...
	void ChannelShuffle::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void ChannelShuffle::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
