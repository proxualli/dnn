...
namespace dnn
{
	ChannelSplit::ChannelSplit(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const size_t group, const size_t groups) :
	...
	std::string ChannelSplit::GetDescription() const
	{
	...
	size_t ChannelSplit::FanIn() const
	{
		return 1;
	}

	size_t ChannelSplit::FanOut() const
	{
		return 1;
	}

	void ChannelSplit::InitializeDescriptors(const size_t batchSize)
	{
	...
	void ChannelSplit::ForwardProp(const size_t batchSize, const bool training)
	{
	...
	void ChannelSplit::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
