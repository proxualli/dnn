...
namespace dnn
{
	Dropout::Dropout(const dnn::Device& device, const dnnl::memory::format_tag format, const std::string& name, const std::vector<Layer*>& inputs, const Float dropout) :
	...
	std::string Dropout::GetDescription() const
	{
	...
	size_t Dropout::FanIn() const
	{
		return 1;
	}

	size_t Dropout::FanOut() const
	{
		return 1;
	}

	void Dropout::InitializeDescriptors(const size_t batchSize)
	{
	...
	void Dropout::SetBatchSize(const size_t batchSize)
	{
	...
	void Dropout::ForwardProp(const size_t batchSize, const bool training)
	{
		if (training)
		{
	...
	void Dropout::BackwardProp(const size_t batchSize)
	{
#ifdef DNN_LEAN
